Feature:CatchyLabs M_WEB Scenarios
  ...
  Scenario: TC01CatchyLabsSuccessfulLoginCheck_MWEB
    Given Check that the account has been successfully logged in

  Scenario: TC02CatchyLabsMoneyTransferProcessCheck_MWEB
  ...
  Scenario: TC03CatchyLabsAccountDepositCheck_MWEB
  ...
  Scenario: TC04CatchyLabsUpdateAccountNameCheck_MWEB
    Given Select Transfer Money
  ...
  Scenario: TC05CatchyLabsInvalidCardNumberCheck_MWEB
    When Save the amount value
    Given Select Transfer Money
      | Transactions My Account | Add Money | Entry Card Number | Entry Card Holder | Entry Expiry Date | Entry Cvv | Entry Add Amount | Add Complete |
